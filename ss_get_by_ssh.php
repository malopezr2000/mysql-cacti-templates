...
# ============================================================================
# This is a script to retrieve information from an Apache server for input to a
...
$ssh_iden = '-i /etc/cacti/id_rsa.pub';   # SSH identity
...
if (!isset($called_by_script_server)) {
   array_shift($_SERVER["argv"]); # Strip off ss_get_apache_stats.php
   $options = parse_cmdline($_SERVER["argv"]);
   validate_options($options);
   $result = ss_get_apache_stats($options);
   if ( !$debug ) {
...
function usage($message) {
   $usage = <<<EOF
$message
Usage: php ss_get_apache_stats.php --host <host> --items <item,...> [OPTION]
...
# top of this file.
# ============================================================================
function ss_get_apache_stats( $options ) {
...
# TODO: convert this to use flock()
      $result = run_query(
   ...
   $iden = $ssh_iden;
   $cmd = "ssh $user@$host -p $port $iden wget -q -O - -T 5 'http://localhost/$url?auto'";
   $str = `$cmd`;
